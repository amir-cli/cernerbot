    ...
    if msgs > max_msg then
	  ...
	  if msg.to.type == 'chat' or msg.to.type == 'channel' then
		if username then
			...
			send_large_msg(receiver , "<b>CerNer Team </b>\n<b>Flooding</b> is not allowed here\n@"..username.." <i>["..msg.from.id.."]</i>\nStatus: User kicked")
		else
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] kicked for #spam")
			send_large_msg(receiver , "<b>CerNer Team </b>\n<b>Flooding</b> is not allowed here\nName: "..name_log.." <i>["..msg.from.id.."]</i>\nStatus: User kicked ")
		end
	  end
      -- incr it on redis
      ...
      if gbanspamonredis then
        if tonumber(gbanspamonredis) ==  4 and not is_owner(msg) then
		  ...
		  for k,v in pairs(GBan_log) do
			log_SuperGroup = v
			gban_text = "این کونی \n[ "..name.." ] ( @"..username.." ) "..msg.from.id.." بن گلوبال شد/:( "..msg.to.print_name.." ) [ "..msg.to.id.." ] (دلیل کس لیسی)"
